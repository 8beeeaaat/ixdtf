...
export const CalendarVersusExactTime: Story = {
  play: async ({ args, canvasElement }) => {
    ...
    await expect(zdt.add({ days: 1 }).toString()).toBe(args.observations.plusDay.formatted);
    await expect(zdt.add({ days: 1 }).epochNanoseconds.toString()).toBe(
      args.observations.plusDay.epochNanoseconds,
    );
    await expect(zdt.add({ hours: 24 }).toString()).toBe(args.observations.plusHours.formatted);
    await expect(zdt.add({ hours: 24 }).epochNanoseconds.toString()).toBe(
      args.observations.plusHours.epochNanoseconds,
    );

    await expect(canvas.getByText("+25 h")).toBeVisible();
    await expect(canvas.getByText("+24 h")).toBeVisible();
    // F-6-5: Temporal の epoch と Go の unix_nano が同値で 2 箇所ずつ現れる
    await expect(
      canvas.getAllByText(args.observations.plusDay.epochNanoseconds ?? ""),
    ).toHaveLength(2);
    await expect(
      canvas.getAllByText(args.observations.plusHours.epochNanoseconds ?? ""),
    ).toHaveLength(2);
