...
| D-13 | リクエストボディ上限 64 KiB + セキュリティヘッダー | 認証・DB なしの公開 API のため、残る攻撃面はリソース消費のみ。controller の `decodeJSON` が `http.MaxBytesReader` で 64 KiB 超を 400 で拒否 (IXDTF 文字列は高々数 KB、A-2 の「リクエスト不備」扱いで契約変更なし)。静的アセットは `web/public/_headers` で CSP / nosniff / frame 拒否を付与、API レスポンスは `writeJSON` が nosniff を付与。CSP の外部許可は Cloudflare Web Analytics の beacon 取得 (`script-src https://static.cloudflareinsights.com`) と送信 (`connect-src https://cloudflareinsights.com`) のみ (D-15) |
...
| D-15 | アクセス解析は Cloudflare Web Analytics、SEO はクライアント側の head 同期 + 静的 robots / sitemap (2026-09-24) | 解析は Cookie・個人データを使わない Cloudflare Web Analytics に限定し、CSP の外部許可を beacon の 2 ホストに絞る (N-8)。SEO (N-7) は SSR / プリレンダリングを導入せず、`lib/useDocumentHead.ts` が `pathname` と UI 言語をキーに title / description / canonical を更新する。画面ごとのメタ情報は `lib/seo.ts` に集約し、`public/sitemap.xml` との一致をテストで保証する。`index.html` には静的な canonical を置かない (全ルートが同じ HTML を返すため、JS を実行しないクローラーに「全画面がトップの重複」と伝わってしまう)。言語は URL で分けていないので hreflang は付けない。旧パスの 301 は `server/worker-entry.mjs` が担う |
