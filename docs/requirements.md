...
- **F-0-1** ヘッダーナビゲーションで 6 画面を切り替えられる SPA であること (ホーム / はじめに / ガイド / ワークベンチ / Temporal ラボ / 変換。旧 Playground+Interop はワークベンチに統合)
...
- **F-0-4** ブラウザの TC39 Temporal 実装をユーザー選択なしで扱うこと (実行時セレクタは撤去)
  ...
  - 比較系 (ワークベンチ / Temporal Lab) はネイティブと polyfill を両方明示併記する
