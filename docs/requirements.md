...
SPA として以下の 7 画面をヘッダーナビゲーションで切り替える。
...
| F-6 | Temporal Lab | IXDTF と Temporal の日時モデルの性質をネイティブ Temporal の実測で観察 (Go ixdtf の実測と再現コードを併記) |
...
- **F-0-1** ヘッダーナビゲーションで 7 画面を切り替えられる SPA であること
- **F-0-2** UI 言語は日本語 / 英語を切り替えられること (react-i18next、全キーは両 locale で同期)
- **F-0-3** ライト / ダークテーマに対応すること (`prefers-color-scheme` 追従 + 手動切替)
- **F-0-4** ブラウザネイティブの TC39 Temporal と temporal-polyfill を実行時に切り替えられること
  - `globalThis.Temporal` の存在を検出し、初期実装を選ぶ (保存された選択があればそれを優先、無ければ
    ネイティブがあればネイティブ、無ければ polyfill をデフォルトとする)。選択は localStorage に記憶する
  - ヘッダーに現在の実装を示す切替コントロールを置き、両実装の挙動を見比べられること
  - ネイティブ非対応ブラウザでネイティブが選択された場合のみ、対応ブラウザ一覧
    (Chrome 144+ / Edge 144+ / Firefox 139+) を示す案内画面を表示し、polyfill への切替を促すこと
- **F-0-5** レスポンシブ対応 (モバイル〜デスクトップ)
- **F-0-6** Playground / Interop / Guide を中心に、専門用語・処理系固有の挙動には文脈に応じた
  「解説・参照」導線を表示できること
  ...
  - 初期配置は Playground / Interop / Guide とし、同じ参照 ID を使って他画面へ拡張できること
  - Temporal の挙動を説明する解説には、実際の結果はブラウザ内蔵実装に依存し、ブラウザや
    バージョンによって未実装・実装差・仕様との差が生じ得ることを明記すること
- **F-0-7** サーバー API を呼び出す画面 (F-1-7 / F-2 / F-3 / F-6) では、結果表示の直下に折りたたみ式の
...
### F-2: IXDTF ワークベンチ — 解析・検証 / 往復比較 (旧 Playground + Interop 統合)

同一の IXDTF 入力を 3 実装 (ネイティブ Temporal / temporal-polyfill / Go サーバー) に通す単一画面。「解析・検証」「往復・実装差比較」の 2 モードを切り替え、入力欄・ハイライト (F-2-5)・3 実装を両モードで共有する。

- **F-2-0** モードを「解析・検証」「往復・実装差比較」で切り替えられること (入力・ハイライト・3 実装は共有)
...
### F-6: Temporal Lab — IXDTF 日時モデル

Interop の実装差比較とは分離し、IXDTF と Temporal が持つ日時モデル上の性質そのものを
小さな実験群として観察する画面。各実験はブラウザネイティブ Temporal の実測値を主役に、
Go ixdtf のライブ実測値と再現コード (F-0-7) を併記する。ただし Interop (F-3) のような
一致・不一致の採点は行わない — 両実装が同じ日時モデルへ到達することを示す教材である。
...
- **F-6-1** DST 終了時に繰り返される同一ローカル時刻を使った「重複時刻」デモを表示すること
  ...
  - Interop の挙動差プリセットとは関連付けず、match / mismatch の採点 UI も置かないこと
...
- **F-7-4** F-0-6 の解説・参照導線 (ReferenceDialog) で一次情報へ誘導し、
  関連画面 (Playground / Interop / Guide / Temporal Lab) への遷移導線を設けること
